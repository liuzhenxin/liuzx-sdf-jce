...
#   PACK_INCLUDE_CONF=1  include the REAL device config (may contain credentials/IPs);
#                        default is a sanitized template, no real IP/credentials
...
PACK_INCLUDE_CONF="${PACK_INCLUDE_CONF:-0}"
...
# Device config: sanitized by default. Real credentials/IPs are only included when
# PACK_INCLUDE_CONF=1, which prints a prominent warning.
# NOTE: CONF_KIND describes how the vendor opens the config (dir vs file) and is not
# the same as whether CONF_SOURCE is a directory — Shudun/SanSec pass a directory but
# ship the ini as a file. Always test the real filesystem type for copying.
IS_CONF_DIR=false
[[ -d "${CONF_SOURCE}" ]] && IS_CONF_DIR=true

CONF_BASENAME="$(basename "${CONF_SOURCE}")"
if [[ "${IS_CONF_DIR}" == "true" ]]; then
    EXPECTED_INI="$(cd "${CONF_SOURCE}" && ls -1 ./*.ini 2>/dev/null | head -1 | xargs -n1 basename 2>/dev/null || true)"
    [[ -n "${EXPECTED_INI}" ]] && CONF_BASENAME="${EXPECTED_INI}"
fi

if [[ "${PACK_INCLUDE_CONF}" == "1" ]]; then
    echo "[pack] WARNING: including real device config (may contain credentials/IPs); do not distribute this bundle" >&2
    if [[ "${IS_CONF_DIR}" == "true" ]]; then
        cp -R "${CONF_SOURCE}/." "${STAGE}/${BUNDLE_NAME}/conf/"
        CONF_HINT="conf/ (real device config included)"
    else
        cp "${CONF_SOURCE}" "${STAGE}/${BUNDLE_NAME}/conf/"
        CONF_HINT="conf/$(basename "${CONF_SOURCE}") (real device config included)"
    fi
else
    # Sanitized: copy only *.ini.example and write a placeholder template.
    if [[ "${IS_CONF_DIR}" == "true" ]]; then
        find "${CONF_SOURCE}" -maxdepth 1 -name '*.ini.example' -exec cp {} "${STAGE}/${BUNDLE_NAME}/conf/" \; 2>/dev/null || true
    else
        EXAMPLE="${CONF_SOURCE}.example"
        [[ -f "${EXAMPLE}" ]] && cp "${EXAMPLE}" "${STAGE}/${BUNDLE_NAME}/conf/"
    fi
    cat > "${STAGE}/${BUNDLE_NAME}/conf/${CONF_BASENAME}" <<'TPL'
# Sanitized template generated by pack-smoke.sh. Fill in <HOST>/<PORT> before running.
[ HSM ]
MODE = 0
IP = <HOST>
PORT = <PORT>
...
TRACK = 0
TPL
    CONF_HINT="conf/${CONF_BASENAME} (sanitized template; fill in <HOST>/<PORT>)"
    echo "[pack] conf sanitized: real device config NOT included (set PACK_INCLUDE_CONF=1 to include it)"
fi

if [[ "${CONF_KIND}" == "dir" ]]; then
    CONF_PROPERTY="\${ROOT_DIR}/conf"
else
    CONF_PROPERTY="\${ROOT_DIR}/conf/${CONF_BASENAME}"
fi
